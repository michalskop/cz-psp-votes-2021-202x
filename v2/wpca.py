...
Xsource = pd.read_csv(path + data_path + "votes.csv")
...
conditions = [
  ...
  Xsource['option'].isin(['before oath', 'absent'])
]
...
Xraw = pd.pivot_table(Xrawdb, values='option_numeric', columns='mp_id', index='vote_event_id', aggfunc='first', fill_value=np.nan)
...
out.columns = ['mp_id', 'dim1', 'dim2', 'dim3']
...
out.to_csv(path + views_path + "wpca.csv", index=False)
...
header = ['mp_id', 'dim1', 'dim2', 'dim3', 'příjmení', 'jméno', 'kandidátka', 'klub', 'datum']
out = pd.DataFrame(columns=header)

mps_out = mps.loc[:, ['mp_id', 'family_name', 'given_name', 'list_name_cs', 'last_group_abbreviation']]
mps_out.columns = ['mp_id', 'příjmení', 'jméno', 'kandidátka', 'klub']

for i, projection in enumerate(projections):
  p = projection.iloc[:, 0:3]
  p.columns = ['dim1', 'dim2', 'dim3']
  p.reset_index(inplace=True)
  p = p.merge(mps_out.loc[:, ['mp_id', 'příjmení', 'jméno', 'kandidátka', 'klub']], on='mp_id', how='left')
  p['datum'] = i
...
out.to_csv(path + views_path + "wpca.halfyear.v1.csv", index=False)