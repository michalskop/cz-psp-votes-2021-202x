...
for interval in intervals:
  ...
  valid_votes_interval = valid_votes[valid_votes['vote_event_id'].isin(vote_events[vote_events['date'] >= interval['start']][vote_events['date'] <= interval['end']]['vote_event_id'])]

  rt = pd.pivot_table(valid_votes_interval, index=['mp_id'], values=['rebeling', 'group_way_abs'], aggfunc=np.sum)
  ...
  gt = pd.pivot_table(valid_votes_interval, index=['mp_id'], values=['against_gov', 'possibly_against_gov'], aggfunc=np.sum)
  ...
  output.to_csv(path + data_path + "partly/rebelity_" + interval['name'] + ".v1.csv", index=False)
  ...
  output2.to_csv(path + data_path + "partly/govity_" + interval['name'] + ".v1.csv", index=False)
