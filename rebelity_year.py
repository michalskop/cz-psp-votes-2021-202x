"""Calculates rebelity and govity for one year."""
...
valid_votes = valid_votes[valid_votes['vote_event_id'].isin(vote_events[mask]['vote_event_id'])]
...
output.to_csv(path + data_path + "rebelity.v1." + str(year) + ".csv", index=False)
...
output2.to_csv(path + data_path + "govity.v1." + str(year) + ".csv", index=False)
